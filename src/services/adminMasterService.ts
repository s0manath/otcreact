...
export const adminMasterService = {
  ...
  getRegions: async (query?: string) => {
    const response = await api.post<RegionMaster[]>(
      "/AdminMaster/regions-list",
      { query },
    );
    return response.data;
  },
  ...
  getKeys: async (query?: string) => {
    const response = await api.post<KeyInventoryMaster[]>(
      "/AdminMaster/key-inventory-list",
      { query },
    );
    return response.data;
  },
  ...
  getPendingRequests: async () => {
    const response = await api.post<PendingLoginRequest[]>(
      "/AdminMaster/pending-requests-list",
    );
    return response.data;
  },
  processRequest: async (
    ...
    const response = await api.post("/AdminMaster/process-request", {
      requestId,
      isApproved,
      comments,
    });
  ...
  getCustodianMappings: async () => {
    const response = await api.post<any[]>(
      "/AdminMaster/mappings/custodian-list",
    );
    return response.data;
  },
  ...
  bulkUpdateRouteKeys: async (atmIds: string[], routeKey: string) => {
    const response = await api.post("/AdminMaster/bulk-update-route-keys", {
      atmIds,
      routeKey,
    });
    return response.data;
