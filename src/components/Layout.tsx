...
const Layout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    const SubMenuLink = ({ label, path }: { label: string; path: string }) => {
        ...
        return (
            <button
                onClick={() => navigate(path)}
                className={`w-full flex items-center justify-between gap-3 px-4 py-2 text-xs font-semibold tracking-wide transition-colors ml-9 border-l-2 ${active
        ...
        <div className="min-h-screen bg-[#f8fafc] text-slate-700 flex font-sans">
            ...
            <aside className="w-72 bg-white border-r border-slate-200 flex flex-col sticky top-0 h-screen overflow-y-auto hide-scrollbar">
                <div className="p-6 flex items-center gap-3 border-b border-slate-50 mb-4 sticky top-0 bg-white z-10 font-black italic">
                    ...
                    <span className="text-xl font-black text-slate-900 tracking-tighter uppercase">OTC Portal</span>
                </div>
                ...
                <div className="flex-1 px-4 py-2 space-y-6">
                    <div>
                        ...
                        <div className="space-y-1">
                            ...
                            <MenuLink icon={<MapPin size={18} />} label="Route Configure" path="/route" />
                        </div>
                    </div>
                        ...
                        <div className="space-y-1">
                            ...
                            >
                                <div className="flex items-center gap-3">
                                    <FileText size={18} />
                                    <span className="text-sm uppercase tracking-wide">System Reports</span>
                                </div>
                    ...
                    <div>
                        ...
                        <div className="space-y-1">
                            ...
                            >
                                <div className="flex items-center gap-3">
                                    <Settings size={18} />
                                    <span className="text-sm uppercase tracking-wide">Admin Masters</span>
                                </div>
                                {isAdminOpen ? <ChevronDown size={14} /> : <ChevronRight size={14} />}
                            </button>
                            ...
                            <AnimatePresence>
                                {isAdminOpen && (
                                    ...
                                    >
                                        ...
                                        <SubMenuLink label="Custodian Master" path="/masters/custodians" />
                                        <SubMenuLink label="ATM Master" path="/masters/atms" />
                                        <SubMenuLink label="Location Master" path="/masters/locations" />
                                        <SubMenuLink label="Franchise Master" path="/masters/franchises" />
                                        <SubMenuLink label="State Master" path="/masters/states" />
                                        <SubMenuLink label="District Master" path="/masters/districts" />
                                        <SubMenuLink label="ZOM Master" path="/masters/zoms" />
                                        <SubMenuLink label="Region Master" path="/masters/regions" />
                                        <SubMenuLink label="Route Master (Admin)" path="/masters/route-admin" />
                                        <SubMenuLink label="Key Inventory" path="/masters/keys" />
                                        <SubMenuLink label="One Line Master" path="/masters/one-line" />
                                        <SubMenuLink label="Site Access Master" path="/masters/site-access" />
                                        <SubMenuLink label="Custodian Mappings" path="/masters/mappings/custodian" />
                                        <SubMenuLink label="ZOM Mappings" path="/masters/mappings/zom" />
                                        <SubMenuLink label="Login Requests" path="/masters/requests" />
                                        <SubMenuLink label="Route Mapping Utility" path="/masters/route-mapping" />
                                    </motion.div>
                                )}
                            </AnimatePresence>
                        </div>
            ...
            <main className="flex-1 overflow-x-hidden flex flex-col">
                <header className="h-20 bg-white/70 backdrop-blur-xl border-b border-slate-200 px-8 flex items-center justify-between sticky top-0 z-20">
                    <div>
                        <h2 className="text-lg font-black text-slate-900 tracking-tight">OTC Modern Intelligence</h2>
                        <p className="text-[10px] text-slate-400 font-black uppercase tracking-widest italic">System v2.1.0-Stable</p>
                    </div>
                    ...
                    <div className="flex items-center gap-5">
                        <div className="hidden lg:flex items-center bg-slate-50 rounded-full px-4 py-2 border border-slate-200 w-80 shadow-inner group">
                            <Search size={16} className="text-slate-400 mr-2 group-focus-within:text-primary-500 transition-colors" />
                            <input type="text" placeholder="Search parameters..." className="bg-transparent border-none focus:outline-none text-xs w-full text-slate-700 font-semibold" />
                        </div>
