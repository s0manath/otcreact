...
const ScheduleBulkUploadModal: React.FC<ScheduleBulkUploadModalProps> = ({
  isOpen,
  onClose,
  onSuccess,
}) => {
              ...
              {status === "success" || (status === "error" && result) ? (
                <div className="space-y-6">
                  <div className="flex flex-col items-center text-center p-6 bg-slate-50 rounded-[2rem] border border-slate-100">
                    <div
                      className={`w-16 h-16 rounded-full flex items-center justify-center text-white mb-4 shadow-xl ${status === "success" && result?.invalidRecords === 0 ? "bg-emerald-500 shadow-emerald-500/20" : "bg-amber-500 shadow-amber-500/20"}`}
                    >
                      {status === "success" && result?.invalidRecords === 0 ? (
                    ...
                    <div className="mt-4 grid grid-cols-3 gap-8 w-full">
                      ...
                      <div>
                        <p className="text-[10px] font-black uppercase tracking-widest text-rose-400 italic">
                          Errors
                        </p>
                ...
                <>
                  ...
                  <div
                    onClick={() =>
                      status === "idle" && fileInputRef.current?.click()
                    }
                    className={`
                      ...
                      {file ? (
                        ...
                        <>
                          ...
                          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-2 leading-relaxed">
                            Select .xlsx or .xls protocol <br /> from local
                            terminal node
                          </p>
                        </>
                      )}
