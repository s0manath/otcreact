...
import masterService, { type DistrictMaster, type MasterDropdownItem } from '../services/masterService';

const DistrictMasterForm: React.FC = () => {
    const navigate = useNavigate();
    const { id } = useParams<{ id: string }>();
    const isEdit = !!id;

    const [form, setForm] = useState<DistrictMaster>({
        id: 0,
        districtName: '',
        stateId: undefined,
        isActive: true
    ...
    const loadData = async () => {
        try {
            setLoading(true);
            const stateData = await masterService.getStates(); // Assuming generic dropdown fetch exists
            setStates(stateData);

            if (isEdit) {
                const data = await masterService.getDistrict(parseInt(id!));
                setForm(data);
            }
        } catch (error) {
    ...
    const handleSave = async (e: React.FormEvent) => {
        ...
        try {
            setSaveLoading(true);
            await masterService.saveDistrict(form);
    ...
    if (loading) {
        return (
            <div className="min-h-screen bg-[#f8fafc] flex flex-col items-center justify-center gap-4">
                ...
                <p className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Loading District Node...</p>
            </div>
        );
    }
                ...
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-12">
                    <motion.div initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }}>
                        ...
                        >
                            ...
                            <span className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 group-hover:text-sky-600">Back to Hierarchy</span>
                        </button>
                        <h1 className="text-4xl font-black text-slate-900 tracking-tight leading-none mb-4">
                            {isEdit ? 'Refine' : 'Add'} <span className="text-sky-600 px-2 py-1 bg-sky-50 rounded-2xl">District Node.</span>
                        </h1>
                    </motion.div>
                </div>
                ...
                <form onSubmit={handleSave} className="bg-white rounded-[2.5rem] border border-slate-200 shadow-xl p-10 space-y-8">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div className="space-y-2">
                            <label className="text-[10px] font-black uppercase tracking-widest text-slate-500 ml-1">District Identifier</label>
                            <input
                                type="text"
                                required
                        ...
                        <div className="space-y-2">
                            <label className="text-[10px] font-black uppercase tracking-widest text-slate-500 ml-1">Parent State</label>
                    ...
                    <div className="flex items-center gap-4 pt-4 border-t border-slate-100">
                        <div className="flex-1">
                            <h4 className="text-xs font-black text-slate-900 uppercase tracking-widest">Node Status</h4>
                            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Operational availability of this district node</p>
                        </div>
                        <button
                            type="button"
