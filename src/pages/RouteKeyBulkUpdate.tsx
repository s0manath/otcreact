...
import { Key, CheckCircle2, ChevronRight, ArrowLeft } from "lucide-react";
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      reader.onload = async () => {
        ...
        const res = await masterService.updateRouteKeyBulk(file.name);
        console.log(res);
        // setResult(res);
        setStatus("success");
      };
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-12 relative">
          ...
          >
            ...
            >
              ...
              />
              <span className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 group-hover:text-orange-600">
                Back to Terminal Control
              </span>
            </button>
            <h1 className="text-4xl lg:text-5xl font-black text-slate-900 tracking-tight leading-none mb-4">
              Route Key <br />
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-orange-600 to-amber-600 text-6xl">
                Propagation.
              </span>
            </h1>
            <p className="text-slate-500 font-bold text-lg max-w-xl">
              Mass synchronize cryptographic route keys across distributed
              terminal nodes. Ensure alignment with security matrices.
            </p>
          </motion.div>
        </div>
                  ...
                  {file ? (
                    ...
                    <div className="text-center">
                      ...
                      <p className="text-slate-400 font-bold mb-8">
                        Select .xlsx file with [ATM ID] and [New Route Key]
                        mappings
                      </p>
                      <button
