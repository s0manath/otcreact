...
const RouteConfigure: React.FC = () => {
    ...
    const fetchRouteData = async () => {
        ...
        if (start > today || end > today) {
            setToast({
                isVisible: true,
                message: "Future dates are not permitted. Please select a date on or before today.",
                type: 'error'
            });
            return;
        }
        ...
        if (diffInDays > 31) {
            setToast({
                isVisible: true,
                message: "Date range cannot exceed 1 month (31 days). Please refine your search.",
                type: 'error'
            });
            return;
        }
    ...
    const columns = [
        {
            ...
            render: (val: any, _row: any) => (
                <span className="font-bold text-slate-800 text-sm tracking-tight capitalize">{val}</span>
            )
        },
        {
            header: 'ATM ID',
        ...
        { header: 'District Name', key: 'district_Name' },
        {
            ...
            key: 'routeKey',
            render: (val: any, _row: any) => (
                <span className="text-[11px] font-black text-slate-900 border border-slate-100 bg-slate-50 px-2 py-0.5 rounded uppercase tracking-widest">{val || 'N/A'}</span>
            )
        },
        {
            header: 'Custodian 1',
            ...
            render: (val: any, _row: any) => (
                <div className="flex flex-col">
                    <span className="text-[10px] font-bold text-slate-600 truncate max-w-[100px]">{val || 'Unassigned'}</span>
                </div>
            )
        },
        {
    ...
    const handleExport = () => {
        alert('Exporting current filter results (Excel)...');
    };
            ...
            <div className="px-8 pt-8 flex items-center justify-between">
                <div className="flex items-center gap-4">
                    ...
                    >
                        ...
                        <Loader2 size={10} className={`text-slate-400 group-hover:text-primary-500 ${loading ? 'animate-spin' : ''}`} />
                    </button>
                </div>
                ...
                <div className="bg-white rounded-[2.5rem] border border-slate-200 shadow-sm overflow-hidden flex flex-col min-h-[400px]">
                        ...
                        <table className="w-full text-left border-collapse min-w-[1400px]">
                            <thead>
                                <tr className="border-b border-slate-100 bg-slate-50/30">
                                    {columns.map(col => (
                                        <th key={col.key} className="px-6 py-5 text-[10px] font-black text-slate-400 uppercase tracking-[0.15em] whitespace-nowrap">{col.header}</th>
                                    ))}
                                    <th className="px-6 py-5 text-[10px] font-black text-slate-400 uppercase tracking-[0.15em] text-right sticky right-0 bg-slate-50/30 backdrop-blur-md border-l border-slate-100">Actions</th>
                                </tr>
                            </thead>
                            <tbody>
                                        ...
                                        {columns.map(col => (
                                            <td key={col.key} className="px-6 py-4 whitespace-nowrap">
                                                {col.render ? col.render(row[col.key as keyof typeof row], row) : (
                                                    <span className="text-[11px] font-bold text-slate-600 tracking-tight">{row[col.key as keyof typeof row] || '-'}</span>
                                                )}
                                            </td>
                                        ))}
                    ...
                    {paginatedData.length === 0 && !loading && (
                        <div className="flex-1 flex flex-col items-center justify-center p-20 text-slate-400 gap-4">
                            ...
                            <p className="text-xs font-black uppercase tracking-widest italic text-center">No routes found matching your filters<br /><span className="text-[10px] opacity-60">Try adjusting your search or advanced filters</span></p>
                        </div>
                    )}
                </div>
            </div>
